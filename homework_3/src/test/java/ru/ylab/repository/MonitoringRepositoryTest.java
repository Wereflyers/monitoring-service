...
class MonitoringRepositoryTest {
    ...
    public void checkIndicationForMonthTest() {
        ...
        when(config.connect()).thenReturn(JDBCConfigTest.connect());
        Long result1 = monitoringRepository.checkIndicationForMonth("first_user", 1L, 2);

        when(config.connect()).thenReturn(JDBCConfigTest.connect());
        Long result2 = monitoringRepository.checkIndicationForMonth("first_user", 1L, 3);

        when(config.connect()).thenReturn(JDBCConfigTest.connect());
        Long result3 = monitoringRepository.checkIndicationForMonth("second_user", 2L, 1);

        assertThat(result1).isEqualTo(indication1.getValue());
        assertThat(result2).isNull();
        assertThat(result3).isEqualTo(indication2.getValue());
    }
